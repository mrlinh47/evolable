...
function query_ajax_pagination( $post_type = 'post', $posts_per_page = 5, $paged = 1){
    ...
    if($q_svl->have_posts()):
        ...
        $allpost .= '<ul class="news-list">';
        while($q_svl->have_posts()):$q_svl->the_post();

            $allpost .= '<li>';
            $allpost .= '<label class="news-date">'.get_the_date('Y.m.d').'</label>';
            $allpost .= '<ul class="categories-list">';?>
                    <?php 
                        ...
                        $allpost .= '<a class="'.$class.'" href="'.$category_link.'">'.$category->cat_name.'</a>';
                     
                     $allpost .= '</li>';  } 
                    ?>
                    <?php
                    $allpost .= '</ul>';
            $allpost .= '<a href="'.get_permalink().'" title="'.get_the_title().'" class="news-title">'.get_the_title().'</a>';
