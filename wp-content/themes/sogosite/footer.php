						...
						<div class="tab-content">
                            ...
                            <div id="tab2" class="tab-pane fade">
                                <ul class="search-job-list">
                                    ...
                                    <?php if ($arlocation->count > 0 && $arlocation->parent != 0 && $arlocation->parent != 353 && $arlocation->parent != 354 && $arlocation->parent != 355 && $arlocation->parent != 163 && $arlocation->parent != 156 && $arlocation->parent != 4) { ?>
			...
			<div class="center">
				<ul class="search-job-list">
				    ...
				    $ppositions      = get_terms($tax_position, $args );
                      ...
                      //var_dump($ppositions);
					?>
                    <?php foreach ($ppositions as $key => $arposition) { ?>
                        <?php if($arposition->parent == 0){ ?>
                        <div style="float: left;width: 100%;border: 1px solid #ccc;padding-bottom:20px">
                            <h3 style="background: #ccc;padding: 5px 0;text-align: center;margin-top:0">
                                <?php echo $arposition->name; ?>
                            </h3>
                                <?php if($arposition->count != 0){ ?>
                                    <form action="<?php echo site_url( '/'.qtranxf_getLanguage().'/jobs/'); ?>" method="get">
                                        <input type="hidden" name="position" value="<?php echo $arposition->term_id ?>">
                                        <li><button type="submit" ><?php echo $arposition->name; ?></button></li>
                                    </form>
                                <?php } ?>
                                <?php unset($ppositions[$key]); ?>
                                <?php foreach ($ppositions as $key1 => $arposition1) { ?>
                                    <?php if($arposition1->parent == $arposition->term_id){ ?>
                                    <?php if($arposition1->count == 0){ ?>
                                        <form action="<?php echo site_url( '/'.qtranxf_getLanguage().'/jobs/'); ?>" method="get">
                                            <input type="hidden" name="position" value="<?php echo $arposition1->term_id ?>">
                                            <li style="background: #878787"><button type="submit" ><?php echo $arposition1->name; ?></button></li>
                                        </form>
                                    <?php }else{ ?>
                                        <form action="<?php echo site_url( '/'.qtranxf_getLanguage().'/jobs/'); ?>" method="get">
                                            <input type="hidden" name="position" value="<?php echo $arposition1->term_id ?>">
                                            <li><button type="submit" ><?php echo $arposition1->name; ?></button></li>
                                        </form>
                                    <?php } ?>
                                    <?php unset($ppositions[$key1]); ?>
                                    <?php } ?>
                                <?php } ?>
                            </form>
                        </div>
