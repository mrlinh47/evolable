		...
		function load_customer_interview(){
			...
			if ( $query->have_posts() ) {
				...
				while ( $query->have_posts() ) : $query->the_post();
					...
					$image_thumb = wp_get_attachment_image_src( $thumb_id, 'thumb-interview-company-page' );
					...
					$htmlRender .= '<div class="box-int clearfix">';
				        ...
				        $htmlRender .= '</div>';
				        if( $link != '#' ) :
				        $htmlRender .= '<a class="int-view-dt" href="'. $link .'" target="_blank">'. __('[:ja]続きを読む[:en]Read more[:vi]Xem chi tiết Blog') .'</a>';
				    	endif;
				    $htmlRender .= '</div>';
				endwhile;
        		wp_reset_postdata();
		...
		function load_candidate_interview(){
			...
			if ( $query->have_posts() ) {
				...
				while ( $query->have_posts() ) : $query->the_post();
					...
					$termCompany = get_the_terms($post->ID, "interviews-company");
					$thumb_id_term = get_term_thumbnail_id($termCompany[0]->term_id);
					$image_term_thumb = wp_get_attachment_image_src( $thumb_id_term, 'full' );
					...
					if($thumb_id_term != ""){
						$img_term_src = '<span class="item-info__logo"><img class="img-responsive" src="'. $image_term_thumb[0] .'" style="max-width:131px" alt=""></span>';
					}else{
						$img_term_src = '<span class="item-info__logo"><img class="img-responsive" src="'. $image_term_thumb[0] .'" style="max-width:131px" alt=""></span>';
					}
